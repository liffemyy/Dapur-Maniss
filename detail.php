...
if (isset($_SESSION['isLoggedIn']) && $_SESSION['isLoggedIn'] == true) {
    ...
    // Path video sesuai ID resep (contoh: uploads/video_1.mp4, video_2.mp4, dst)
    $videoPath = "uploads/video_$id.mp4";

    if (file_exists($videoPath)) {
        echo "
        <div style='text-align:center; margin:20px 0;'>
            <video width='80%' controls style='border-radius:10px; box-shadow:0 2px 5px rgba(0,0,0,0.2);'>
                <source src='$videoPath' type='video/mp4'>
                Browser kamu tidak mendukung video.
            </video>
        </div>";
    } else {
        echo "<p style='text-align:center; color:#777;'>Belum ada video tutorial untuk resep ini.</p>";
    }

} else {
    // Pesan kalau belum login
    echo "
    <p style='text-align:center; color:#c45b4d; margin:20px 0;'>
        ⚠️ Login dulu untuk menonton video tutorial.<br>
        <a href='login.php?redirect=detail.php?id=$id' style='color:#c45b4d; font-weight:bold;'>Klik untuk Login</a>
    </p>";
}
?>
